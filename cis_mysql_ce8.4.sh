#!/usr/bin/env bash
# check_rules_ce84_full.sh
# CIS Oracle MySQL Community Server 8.4 Benchmark v1.0.0 (Automated checks)
# Audit-first; adds remediation across all feasible sub-points.
# Static/startup-only or risky changes are emitted as config/SQL to review (and optionally apply via explicit flags).

set -euo pipefail

# ------------------------ Runtime knobs ------------------------
PROFILE="${PROFILE:-L1}"            # L1|L2
SCOPE="${SCOPE:-linux}"             # linux|rdbms
...
REM_INSTR_OUT="${REM_INSTR_OUT:-remediation_instructions.txt}"  # generated my.cnf instructions & ops notes

# Optional riskier categories (apply ONLY if set to YES)
REM_EDIT_CONFIG="${REM_EDIT_CONFIG:-NO}"              # sed-comment passwords in my.cnf (backup first)
REM_HARDEN_PLUGIN_DIR="${REM_HARDEN_PLUGIN_DIR:-NO}"  # chmod/chown plugin_dir
...
REM_REQUIRE_X509="${REM_REQUIRE_X509:-NO}"            # ALTER USER ... REQUIRE X509 for remote accounts lacking SSL
REM_ENFORCE_SSL_REMOTE="${REM_ENFORCE_SSL_REMOTE:-NO}"# ALTER USER ... REQUIRE SSL for remote users lacking ssl_type
REM_FIX_WILDCARDS="${REM_FIX_WILDCARDS:-NO}"          # attempt to remediate wildcard hosts (requires WILDCARD_TARGET_HOST)
REM_FIX_REPL_SUPER="${REM_FIX_REPL_SUPER:-NO}"        # REVOKE SUPER from replication users
...
emit(){ write_head; printf "%s,%s,%s,%s\n" "$1" "$2" "$3" "${4//,/;}" | tee -a "$REPORT"; }
...
ensure_plugin_active(){
  ...
  if [[ "$status" != "ACTIVE" ]]; then
    if [[ "$REMEDIATION" == "YES" ]]; then
      run_sql "INSTALL PLUGIN ${plugin} SONAME '${plugin}.so';" || return 1
    else
      return 1
    fi
  fi
...
# FS helpers
check_file_perm(){ local p="$1" m="$2" o="$3" g="$4"; local s; s="$(stat -Lc '%a %U %G' "$p" 2>/dev/null || true)"; [[ -z "$s" ]] && echo "MISSING" && return 2; read -r am ao ag <<<"$s"; [[ "$am" == "$m" && "$ao" == "$o" && "$ag" == "$g" ]] && echo "OK" || echo "BAD ($am $ao $ag)"; }
fix_file_perm(){ [[ "$REMEDIATION" == "YES" ]] && sudo chown "$3:$4" "$1" 2>/dev/null || true; [[ "$REMEDIATION" == "YES" ]] && sudo chmod "$2" "$1" 2>/dev/null || true; }
...
c1_2(){
  ...
  in_scope_linux || { emit "1.2" "$t" "N/A" "scope=rdbms"; return; }
  ps -eo user,comm | awk '$2~/(mysqld)/{print $1}' | grep -qx mysql && emit "1.2" "$t" "PASS" "mysqld user=mysql" || emit "1.2" "$t" "FAIL" "mysqld not running as mysql"
}

c1_3_1_4_1_6(){
  local t="1.3/1.4/1.6 Disable history & ensure MYSQL_PWD unused"
  in_scope_linux || { emit "1.3-1.6" "$t" "N/A" "scope=rdbms"; return; }
  local hf envc profc
